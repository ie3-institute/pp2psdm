...
from pypsdm import GridContainer, RawGridElementsEnum, SystemParticipantsEnum
...
class ExtEntityMapping:
    ...
    def from_csv(
        ...
        file_path = utils.get_file_path(path, file_name)
        if os.path.exists(file_path):
            return ExtEntityMapping(read_csv(path, file_name, delimiter).set_index("uuid"))
        else:
            if must_exist:
    ...
    def get_file_name() -> str:
        return f"ext_entity_mapping.csv"
